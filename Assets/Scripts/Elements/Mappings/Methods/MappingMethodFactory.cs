...
namespace VRVis.Mappings.Methods {
    ...
    public class MappingMethodFactory {
        ...
        private static Color_Scale CreateColorScale(string methodName, JObject o) {
            ...
            if (o["from"] != null) { fromColor = Utility.ColorFromString((string) o["from"], ',', 0.1f); }
            if (o["to"] != null) { toColor = Utility.ColorFromString((string) o["to"], ',', 0.1f); }
            if (o["neutral"] != null) { neutralColor = Utility.ColorFromString((string) o["neutral"], ',', 0.1f); }
