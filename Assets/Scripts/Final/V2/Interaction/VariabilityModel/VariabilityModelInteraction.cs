...
namespace VRVis.Interaction {
    ...
    /// This script should be attached to each node of the spawned model (attach to prefabs).
    /// </summary>
    public class VariabilityModelInteraction : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
        ...
        void Awake() {
            ...
            if (!hoverUIInstance) {
                ...
                hoverUIInstance.SetActive(false); // hide
        ...
        // ToDo: replace by new system used by structure
        ...
        /// <summary>
        /// Receive event from laser pointer that the pointer exit.
        /// </summary>
        ...
        private void MakeUIFaceUser() {
            ...
            // version 1: move and rotate towards cam
            /*
            // set position in direction of camera
            Transform userCam = Camera.current.transform;
            Vector3 dirToUser = userCam.position - transform.position;
            Vector3 newUIPos = transform.position + dirToUser * distFromCenter;
            hoverUIInstance.transform.position = newUIPos;

            // rotate towards camera
            Vector3 v = userCam.transform.position - transform.position;
            hoverUIInstance.LookAt(userCam.transform.position - v);
            hoverUIInstance.Rotate(rotationOffset);
            */


            // version 2: rotate towards cam and move
